...
const TopNavigation = () => {
  const { user, userProfile, session, logout, isAdmin, isManager, isAuthenticated, isLoading, isInitialized } = useConsolidatedAuth();
  const navigate = useNavigate();
  const location = useLocation();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [debugMode, setDebugMode] = useState(false);

  // Safe auth checks with proper session validation - gate UI until session exists
  const safeIsAdmin = () => {
    try {
      // Ensure we have user, session, and userProfile before checking permissions
  ...
  // Safe user field access - never access user fields directly without checking
  const getUserDisplayName = () => {
    if (!user || !session) return 'User';
    
    // Safe access to user fields with fallbacks
    if (user.email) {
      return user.email.charAt(0).toUpperCase();
    }
    
    if (userProfile?.role) {
      return userProfile.role.charAt(0).toUpperCase();
    }
  ...
  const getUserEmail = () => {
    if (!user || !session) return 'No email';
  ...
  useEffect(() => {
    if (debugMode) {
      console.log('TopNavigation Debug:', {
        ...
        roleCheck: safeIsAdmin() ? 'Admin' : safeIsManager() ? 'Manager' : 'Employee'
      });
    }
  }, [isAuthenticated, isLoading, isInitialized, user, session, userProfile, debugMode]);
  ...
  // Add admin section if user is admin (with proper session validation)
  if (safeIsAdmin()) {
  ...
  const canAccessRoute = (requiredRole: string | null) => {
    ...
    // Ensure user is authenticated and has valid session before checking roles
    if (!isAuthenticated || !user || !session || !userProfile) return false;
    ...
    // Default to allowing access for authenticated users with valid sessions and profiles
    return true;
  };
  ...
  // Don't render navigation if not authenticated or missing session
                ...
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="flex items-center space-x-2 px-2 py-1.5 h-auto min-w-0 max-w-fit">
                    ...
                    <div className="hidden xl:block text-left min-w-0 max-w-32">
                      ...
                      <p className="text-xs text-gray-500 leading-none mt-0.5 truncate">{getUserEmail()}</p>
                    </div>
        ...
        {debugMode &&
          <div className="bg-yellow-50 border-b border-yellow-200 px-4 py-2">
            <div className="text-xs text-yellow-800">
              ...
              User: {getUserRole()} | Email: {getUserEmail()}
            </div>
          </div>
        }
      </header>
