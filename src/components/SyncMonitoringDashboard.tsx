...
const SyncMonitoringDashboard: React.FC = () => {
  ...
  const getRealTableCount = async () => {
    try {
      ...
      const { data: auditData, error: auditError } = await window.ezsite.apis.tablePage(12706, { PageNo: 1, PageSize: 50, OrderByField: 'event_timestamp', IsAsc: false, Filters: [{ name: 'action_performed', op: 'StringContains', value: 'sync' }] });let realLogs: SyncLog[] = [];if (!auditError && auditData?.List) {
