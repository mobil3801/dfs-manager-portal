...
const ProductFileUpload: React.FC<ProductFileUploadProps> = ({ onDataImport, disabled = false }) => {
  ...
  const validateAndCheckDuplicates = (importedData: any[], existingProducts: any[]): ParsedProduct[] => {
    ...
    setIsOpen(false);setFile(null);setParsedProducts([]);setShowPreview(false);};const handleCloseDialog = () => {setIsOpen(false);setFile(null);setParsedProducts([]);setShowPreview(false);};const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {const selectedFile = e.target.files?.[0];if (selectedFile) {if (!selectedFile.name.endsWith('.csv')) {toast({ variant: "destructive", title: "Invalid File Type", description: "Please select a CSV file." });return;
      }
