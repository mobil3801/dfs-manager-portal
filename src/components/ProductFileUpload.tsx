...
const ProductFileUpload: React.FC<ProductFileUploadProps> = ({ onDataImport, disabled = false }) => {
  ...
  const validateAndCheckDuplicates = (importedData: any[], existingProducts: any[]): ParsedProduct[] => {
    ...
    return importedData.map((row, index) => {
      ...
      const unitPrice = mapped.unit_price || 0;const retailPrice = mapped.retail_price || 0;const profitMargin = calculateProfitMargin(unitPrice, retailPrice);
      // Calculate overdue status
      const isOverdue = calculateOverdueStatus(
        mapped.last_shopping_date,
      ...
      if (profitMargin > 0) {
        mapped.profit_margin = profitMargin;
      }
      mapped.overdue = isOverdue;
      mapped.updated_at = new Date().toISOString();
      mapped.last_updated_date = new Date().toISOString();

      const isDuplicate = isDuplicateNameInDB || isDuplicateNameInImport || isDuplicateWeightInDB || isDuplicateWeightInImport;
