...
const PerformanceMonitoringSystem: React.FC = () => {
  ...
  */const analyzeMetrics = useCallback((currentMetrics: PerformanceMetrics) => {const newAlerts: Alert[] = []; // Memory alerts
      ...
      if (currentMetrics.timing.apiResponseTime > 2000) {newAlerts.push({ id: `api-slow-${Date.now()}`, type: 'warning', title: 'Slow API Response', message: `Average API response time is ${currentMetrics.timing.apiResponseTime.toFixed(0)}ms`, timestamp: Date.now(),
            severity: 'medium'
          });
      }
