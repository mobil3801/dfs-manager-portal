...
const PerformanceMonitoringSystem: React.FC = () => {
  ...
  */const analyzeMetrics = useCallback((currentMetrics: PerformanceMetrics) => {const newAlerts: Alert[] = []; // Memory alerts
      if (currentMetrics.memory.percentage > 80) {newAlerts.push({ id: `memory-high-${Date.now()}`, type: 'warning', title: 'High Memory Usage', message: `Memory usage is at ${currentMetrics.memory.percentage.toFixed(1)}%`, timestamp: Date.now(), severity: currentMetrics.memory.percentage > 90 ? 'critical' : 'high', autoResolve: true
          });
      }
