...
const PerformanceMonitoringSystem: React.FC = () => {
  ...
  */const analyzeMetrics = useCallback((currentMetrics: PerformanceMetrics) => {const newAlerts: Alert[] = []; // Memory alerts
      ...
      if (currentMetrics.network.errorRate > 10) {newAlerts.push({ id: `network-errors-${Date.now()}`, type: 'error', title: 'High Network Error Rate', message: `Network error rate is ${currentMetrics.network.errorRate.toFixed(1)}%`,
            timestamp: Date.now(),
            severity: 'high'
          });
      }
