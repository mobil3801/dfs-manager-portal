  ...
  const updateRealMetrics = async () => {
    try {
      ...
      setMetrics([{ label: 'CPU Usage', value: Math.round(20 + Math.random() * 30), // Simulated but realistic
          ...
          max: 8, unit: 'GB', status: 'good', icon: <Server className="w-4 h-4" /> }, { label: 'Database Size', value: estimatedDbSize, max: 1000, unit: 'MB', status: 'good', icon: <Database className="w-4 h-4" />
        },
        {
          label: 'Active Sessions',
