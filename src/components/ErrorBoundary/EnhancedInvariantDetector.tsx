...
const EnhancedInvariantDetector: React.FC = () => {
  ...
  useEffect(() => {const originalError = console.error;const originalWarn = console.warn;console.error = (...args) => {const errorMessage = args.join(' '); // Detect the specific "Invariant failed" error
      ...
      const handleRejection = (event: PromiseRejectionEvent) => {const reason = String(event.reason);if (reason.includes('Invariant') || reason.includes('invariant')) {addViolation({ type: 'react-error', severity: 'high', message: `Unhandled Promise Rejection (Invariant): ${reason}`, fixSuggestion: 'Handle promises properly and check for async state updates.' });}};globalThis.addEventListener('unhandledrejection', handleRejection); // Global error handler
      const handleGlobalError = (event: ErrorEvent) => {const message = event.message || String(event.error);if (message.includes('Invariant') || message.includes('invariant')) {addViolation({ type: 'react-error', severity: 'critical', message: `Global Error (Invariant): ${message}`, stackTrace: event.error?.stack, fixSuggestion: 'Check the stack trace for the source of the invariant violation.' });}};globalThis.addEventListener('error', handleGlobalError);return () => {console.error = originalError;console.warn = originalWarn;globalThis.removeEventListener('unhandledrejection', handleRejection);globalThis.removeEventListener('error', handleGlobalError);};}, [addViolation]);useEffect(() => {if (!isActive) return; // Initial scan
