...
const EnhancedInvariantDetector: React.FC = () => {
  ...
  const detectFiberInconsistencies = useCallback(() => {const violations: Omit<InvariantViolation, 'id' | 'timestamp'>[] = [];try {const reactRoots = document.querySelectorAll('[data-reactroot], #root, [id*="react"]');reactRoots.forEach((root) => {try {const fiber = (root as any)._reactInternalFiber || (root as any).__reactInternalInstance || (root as any)._reactRootContainer;if (fiber) {// Check for common fiber inconsistencies
                const checkFiber = (fiberNode: any, depth = 0) => {if (!fiberNode || depth > 50) return; // Prevent infinite recursion
                  try {// Check for null child references that should exist
                    ...
                    if (fiberNode.return && fiberNode.return.child === fiberNode && fiberNode.sibling === fiberNode) {violations.push({ type: 'fiber-error', severity: 'critical', message: 'Circular reference detected in React Fiber tree', fixSuggestion: 'Check for improper component nesting or state mutations during render.' });}
  ...
  const trackComponentRenders = useCallback(() => {renderCountRef.current++; // If render count is excessive, it might indicate infinite render loops
      if (renderCountRef.current > 1000) {addViolation({ type: 'react-error', severity: 'high', message: `Excessive render cycles detected (${renderCountRef.current}). Possible infinite render loop.`, fixSuggestion: 'Check for state updates during render or missing dependencies in useEffect.' });
  ...
  const scanForViolations = useCallback(() => {
    ...
    try {
      ...
      ...detectFiberInconsistencies()];


      newViolations.forEach((violation) => addViolation(violation));
      setScanCount((prev) => prev + 1);

      if (newViolations.length > 0) {
        setErrorCount((prev) => prev + newViolations.length);
      }
    } catch (error) {
      console.error('Error during violation scan:', error);
      addViolation({
        type: 'unknown',
        severity: 'medium',
        message: `Scanner error: ${error instanceof Error ? error.message : 'Unknown error'}`,
        fixSuggestion: 'Check browser console for detailed error information.'
      });
    }
  }, [isActive, detectDuplicateKeys, detectInvalidNesting, detectFiberInconsistencies, addViolation, trackComponentRenders]);

  // Enhanced error boundary integration with React error capture
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(scanForViolations, 2000);
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      mutations.forEach((mutation) => {
        ...
        mutation.attributeName?.startsWith('data-react'))) {
          shouldScan = true;
        }
      });

      if (shouldScan) {
        setTimeout(scanForViolations, 100);
      }
    });
    ...
    observer.observe(document.body, {
      ...
      subtree: true,
      attributes: true,
      attributeFilter: ['class', 'key', 'data-key', 'style', 'data-react-key']
    });
  ...
  }, [isActive, scanForViolations]);

  const clearViolations = () => {
    setViolations([]);
    setScanCount(0);
    setErrorCount(0);
    setLastInvariantError(null);
    renderCountRef.current = 0;
    toast({
      title: "Violations Cleared",
      description: "All detected violations have been cleared"
    });
  };
