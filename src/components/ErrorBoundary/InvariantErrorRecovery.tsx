...
class InvariantErrorRecovery extends Component<Props, State> {
  ...
  private fixRenderIssues = async (): Promise<void> => {
    return new Promise((resolve) => {
      ...
      const elementsWithListeners = document.querySelectorAll('[onclick], [onchange], [onsubmit]');elementsWithListeners.forEach((element) => {element.removeAttribute('onclick');element.removeAttribute('onchange');element.removeAttribute('onsubmit');});console.log('Fixed render issues');resolve();});};private handleManualRetry = () => {this.setState({
      hasError: false,
      error: null,
      errorInfo: null,
      isRecovering: false,
  ...
  render() {
    if (this.state.hasError) {
      const isInvariantError = this.state.error ? this.isInvariantError(this.state.error) : false;
...
export default InvariantErrorRecovery;