...
export const DeviceAdaptiveProvider: React.FC<DeviceAdaptiveProviderProps> = ({ children }) => {
  ...
  const getUIPreferences = (device: DeviceInfo) => {
    ...
    if (device.isMobile || device.screenSize === 'small') preferredCardSize = 'compact';else
    if (device.isDesktop && device.screenSize === 'xl') preferredCardSize = 'large';

    // Navigation style based on device capabilities and screen size
    let preferredNavigation: 'sidebar' | 'bottom' | 'top' = 'sidebar';
    if (device.isMobile) preferredNavigation = 'bottom';else
    if (device.isTablet || device.screenSize === 'medium') preferredNavigation = 'top';
    ...
    if (device.isMobile && device.screenSize === 'small') optimalFontSize = 'small';else
    if (device.isDesktop && device.screenSize === 'xl') optimalFontSize = 'large';
    ...
    if (device.prefersReducedMotion) animationDuration = 0;else
    if (device.connectionType === 'slow') animationDuration = 150;else
    if (device.deviceType === 'mobile') animationDuration = 250;
    ...
    if (device.isMobile) contentPadding = 'p-4';else
    if (device.isDesktop && device.screenSize === 'xl') contentPadding = 'p-8';

    // Spacing between elements
    let spacing = 'space-y-6';
    if (device.isMobile) spacing = 'space-y-4';else
    if (device.isDesktop && device.screenSize === 'xl') spacing = 'space-y-8';

    // Border radius based on device
    let borderRadius = 'rounded-lg';
    if (device.isMobile) borderRadius = 'rounded-md';else
    if (device.isDesktop) borderRadius = 'rounded-xl';

    // Shadow intensity based on device capabilities
    let shadowIntensity: 'none' | 'subtle' | 'normal' | 'strong' = 'normal';
    if (device.connectionType === 'slow' || device.prefersReducedMotion) shadowIntensity = 'subtle';else
    if (device.isHighDPI && device.isDesktop) shadowIntensity = 'strong';else
    if (device.isMobile) shadowIntensity = 'subtle';
