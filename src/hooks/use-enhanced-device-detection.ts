...
export const useEnhancedDeviceDetection = (): DeviceInfo => {
  ...
  useEffect(() => {
    const detectDevice = () => {
      ...
      if (isMobile || (hasTouch && width <= 767)) deviceType = 'mobile';
      else if (isTablet || (hasTouch && width <= 1024)) deviceType = 'tablet';
      ...
      if (connection) {
        ...
        if (effectiveType === '4g' || downlink > 10) connectionType = 'fast';
        else if (effectiveType === '3g' || effectiveType === 'slow-2g' || downlink < 1.5) connectionType = 'slow';
        else connectionType = 'fast'; // Default to fast for unknown good connections
      }

      // Calculate appropriate navigation height based on device
      let navigationHeight = 64; // Default desktop
      if (deviceType === 'mobile') navigationHeight = 56;
      else if (deviceType === 'tablet') navigationHeight = 60;
