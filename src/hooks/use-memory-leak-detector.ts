...
config: MemoryLeakConfig = DEFAULT_CONFIG)
{
  ...
  const monitorClosureSize = useCallback((closureName: string, closure: any) => {
    ...
    }}, [componentName, config.warnOnLargeClosure, config.maxClosureSize]);return { safeSetTimeout, safeSetInterval, safeAddEventListener, trackSubscription, trackAsyncOperation, safeSetState, monitorClosureSize, cleanup: { timers: cleanupTimers,
