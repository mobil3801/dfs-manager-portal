...
class ClickSendSMSService {
  ...
  private async checkDailyLimit(): Promise<void> {
    try {
      ...
      if (data?.List && data.List.length > 0) {
        ...
        const { data: historyData } = await globalThis.ezsite.apis.tablePage(24202, {
          PageNo: 1,
          PageSize: 1,
          OrderByField: 'id',
          IsAsc: false,
  ...
  }private async logSMSHistory(historyData: any): Promise<void> {try {await globalThis.ezsite.apis.tableCreate(24202, { ...historyData, sent_by_user_id: 1 // This should be the current user ID
